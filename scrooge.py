...
class Scrooge():
    """ Trusted central authority that creates and manages the blockchain. """
    ...
    def add_genesis_block(self):
        """ 
        Add the genesis block to the blockchain. Return 
    ...
    def create_coins(self, coins):
        ...
        """
        tx = CoinCreation(created_coins=coins)
        block = Block(tx)
    ...
    def verify_signatures(self, transaction, signatures):
        ...
        # Verify if all users whose coins will be consumed
        # have signed the payment
        users = []
        for verifying_key, signature in signatures:
            wallet_id = self.wallet.get_wallet_id_from_verifying_key(
                verifying_key)
